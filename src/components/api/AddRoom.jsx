...
const AddRoom = ({ buildingName, floorName, campusId }) => {
  const { user } = useContext(AuthContext);

  const { fetchRooms, buildings, floors } = useSchool();
      ...
      <div>
        ...
        >
          <DialogTrigger className="flex w-full justify-center gap-1 rounded bg-blue-600 p-3 text-white hover:bg-blue-700 md:w-auto md:justify-normal">
            <AddDepartmentIcon />
            <span className="max-w-[8em]">Add Room </span>
          </DialogTrigger>
          <DialogContent className="max-w-[40em] rounded-sm border border-stroke bg-white p-4 !text-black shadow-default dark:border-strokedark dark:bg-boxdark dark:!text-white">
            <DialogHeader>
              <DialogTitle className="text-2xl font-medium text-black dark:text-white">
                Add new Room ({buildingName} - {floorName})
              </DialogTitle>
                  ...
                  <div className="p-6.5">
                    ...
                    <div className="mb-4.5 w-full">
                      ...
                      <input
                        ...
                        value={
                          floors.find(
                            (floor) => floor.floorName.toString() === floorName,
                          )?.floorName || ""
                        }
                        disabled
                      />
